          ...
          <div class="panel-body">                 
            <table class="table table-bordered" id="video-list">
              <?php $video_files = $this->db->get_where('video_file', array('videos_id'=> $param1))->result_array();
               ...
               ?>
               <tr id="row_<?php echo $video_file['video_file_id']; ?>"><td><strong><?php echo $video_file['file_source']; ?></strong></td><td><?php echo $video_file['file_url']; ?></a></td><td><a title="delete" class="btn btn-icon" onclick="delete_row('video_file',<?php echo $video_file['video_file_id'];?>)" class="delete"><i class="fa fa-remove"></i></td></tr>             
            <?php endforeach; ?>
            </table>
            <?php echo form_close(); ?>          
        </div>
          ...
          <div class="panel-body">                
            <table class="table table-bordered" id="download-link-list">
              <?php $download_links = $this->db->get_where('download_link', array('videos_id'=> $param1))->result_array();
               ...
               ?>
              <tr id="row_<?php echo $download_link['download_link_id']; ?>"><td><a href="<?php echo $download_link['download_url']; ?>"><strong><?php echo $download_link['link_title']; ?></strong></a></td><td><a href="<?php echo $download_link['download_url']; ?>"><?php echo $download_link['download_url']; ?></a></td><td><a title="<?php echo tr_wd('delete'); ?>" class="btn btn-icon" onclick="delete_row(<?php echo " 'download_link' ".','.$download_link['download_link_id'];?>)" class="delete"><i class="fa fa-remove"></i></a></td></tr>
            <?php endforeach; ?>
            </table>       
        </div>
      </div>
       ...
       $("#add-link").click(function(){
        ...
        if (isUrl(url)==true && url !='' && type!='') {
            $.ajax({
                ...
                data: "videos_id="+videos_id+"&type=" + type + "&url=" + url,
                dataType: 'json',
       ...
       $("#add-download-link").click(function(){
        ...
        if (isUrl(download_url)==true && download_url !='' && link_title !='') {
            $.ajax({
                ...
                data: "videos_id="+videos_id+"&link_title=" + link_title + "&download_url=" + download_url,
                dataType: 'json',
                success: function(response) {
                    var post_status = response.post_status;
                    var row_id = response.row_id;
