...
class Login extends CI_Controller{
    ...
    function signup($param1='', $param2='')  {
        if ($param1 == 'do_signup') {
            ...
            if($user_exist){
                $this->session->set_flashdata('error', 'Registrace selhala. Uživ. jméno nebo email již existuje.');
                ...
                $this->session->set_flashdata('success', 'Registrace proběhla v pořádku. Nyní se můžete přihlásit.');
                $response['login_status']       = $login_status;
            ...
            $data['page_title']     = 'Přidej se k nám ';            
    ...
    function ajax_signup()  {
        ...
        if($username !='' && $email != '' && $_POST["password"] !='' && $username !=NULL && $email != NULL && $_POST["password"] !=NULL)
        {
            ...
            if($user_exist){
                ...
                $this->load->model('email_model');
                //$this->email_model->account_opening_email($username, $email, $password);
                $this->session->set_flashdata('success', 'Registrace proběhla úspěšně. Nyní se můžete přihlásit.');
    ...
    function forget_password($param1='', $param2='') {
        if ($param1 == 'do_reset') {           
            ...
            if($user_exist){                
                ...
                $this->session->set_flashdata('success', 'Prosím zkontrolujte svůj Email pro dokončení resetu hesla.');
                redirect(base_url() . 'login', 'refresh');                
            }else{
            $this->session->set_flashdata('error', 'Email nebyl nalezen');            
            redirect(base_url() . 'login', 'refresh');
            }    
    ...
    function complete_reset($param1='', $param2='') {
        if ($param1 == 'save') {
            ...
            if($token !='' && $password !='' && $password2 !='' && $password==$password2){
                ...
                $this->db->update('user', $data);
                $this->session->set_flashdata('success', 'Heslo bylo změněno');
                redirect(base_url() . 'login', 'refresh');
            }
            ...
            if(isset($token) && $token !=''){
                ...
                $this->session->set_flashdata('error', 'Špatný token..');
